  ...
  useEffect(() => {
    const checkForMaintenanceOff = async () => {
      ...
      try {
        ...
        if (messages.length > lastMessageCount) {
          ...
          for (const message of newMessages) {
            if (message.trim() === "!mantenimiento off") {
              console.log("[v0] Maintenance OFF command detected, exiting maintenance mode")
        ...
        console.error("[v0] Error checking for maintenance off:", error)
      }
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (command.trim() === "!mantenimiento off") {
          console.log("[v0] Sent maintenance OFF command, waiting for detection...")
        }
      }
    } catch (error) {
